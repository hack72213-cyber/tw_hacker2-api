...
def get_user():
    ...
    # Original API call with proper headers
    headers = {"User-Agent": "Mozilla/5.0"}
    r = requests.get(f"https://users-xinfo-admin.vercel.app/api?key=demo&type=user&term={term}", headers=headers, timeout=15)
    data = r.json()
...
def get_mobile():
    ...
    headers = {"User-Agent": "Mozilla/5.0"}
    r = requests.get(f"https://users-xinfo-admin.vercel.app/api?key=demo&type=mobile&term={term}", headers=headers, timeout=15)
    data = r.json()
    data["developer"] = "@tw_hacker2"
    return jsonify(data)
